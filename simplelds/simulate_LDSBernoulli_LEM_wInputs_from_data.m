...
data = load('data/datasetD_q1/best-lds-4.mat');
nz = size(data.A,1);  % dimensionality of latent z
ny = size(data.C,1);  % dimensionality of observation y
nu = size(data.B,2); % dimensionality of inputs
nT = size(data.y,1);  % number of time bins
maxEMiters = 1; % number of iterations of EM
display_frequency = 1; % frequency with which to print EM results
initialize = 1; % 0 means random and 1 means ssid
...
optsEM.display = display_frequency;  % display frequency
...
elseif initialize == 1
    if optsEM.update.A, mm0.A = data.Ahat; end % initial A param
    if optsEM.update.C, mm0.C = data.Chat; end % initial C param
    if optsEM.update.Q, mm0.Q = Q*2; end % initial Q param
    if optsEM.update.B, mm0.B = data.Bhat; end % initial B param
    if optsEM.update.D, mm0.D = data.Dhat; end % initial D param
end
...
plot(1:nEM,logEvTrace1); box off; 
