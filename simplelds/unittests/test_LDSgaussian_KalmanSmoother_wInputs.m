% test_LDSgaussian_KalmanSmoother_wInputs.m
%
% Unit test for Kalman Filter-Smoother for Gaussian LDS data with external inputs. 
%
...
if abs(loglitrue1-loglitrue2)>TOL
    warning('test_LDSgaussian_KalmanSmoother_wInputs.m unit test FAILED: log-li vals don''t match');
else
    fprintf('test_LDSgaussian_KalmanSmoother_wInputs.m PASSED: log-li vals match\n');
end
...
if maxabsdiff > TOL
    warning('test_LDSgaussian_KalmanSmoother_wInputs.m unit test FAILED: posterior means don''t match');
else
    fprintf('test_LDSgaussian_KalmanSmoother_wInputs.m PASSED: posterior means match\n');
end
...
if max(abs(loglitrue1-loglitrue2))>TOL
    warning('test_LDSgaussian_KalmanSmoother_wInputs.m unit test FAILED: covariances don''t match');
else
    fprintf('test_LDSgaussian_KalmanSmoother_wInputs.m PASSED: covariances match\n');
end
